...
namespace GLXEngine
{
	...
	public class Sprite : GameObject
	{
		...
		virtual public int width {
			get { 
				if (_texture != null) return (int)Math.Abs(_texture.width * scaleX);
				return 0;
			}
			set {
		...
		virtual public int height {
			get { 
				if (_texture != null) return (int)Math.Abs(_texture.height * scaleY);
				return 0;
			}
			set {
		...
		override protected void RenderSelf(GLContext glContext) {
			if (game != null) {
				...
				if (test == false) {
					...
					_texture.Bind();
					glContext.DrawQuad(GetArea(), new float[] { (_color >> 16) & 0xff, (_color >> 8) & 0xff, _color & 0xff, 1f }, _uvs);
					_texture.Unbind();
