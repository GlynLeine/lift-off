...
{
    /// <summary>
    /// Call the method TiledParser.ReadMap, with as argument a Tiled file exported as xml (file extension: .tmx),
    ...
    public class TiledParser
    {
        ...
        {
            serial = new XmlSerializer(typeof(Map));
        ...
        {
            serial = new XmlSerializer(typeof(TileSet));
            TextReader reader = new StreamReader(filename);
            var deserializedObject = serial.Deserialize(reader);
            TileSet tileSet = deserializedObject as TileSet;
            reader.Close();
            return tileSet;
        }
